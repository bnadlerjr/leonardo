...
return size;}
return{columnChart:function(domID,series,labels,options){init(domID,getMaxValue(series),labels.length,options||defaultOpts);if(opts.drawBorder){drawBorder(opts.width,opts.height);}
...
paper.rect(x,y,opts.barWidth,adjustedHeight-y).hover(function(event){this.attr({fill:"gray"});},function(event){this.attr({fill:localColor});}).attr({title:item+": "+value,fill:localColor});});seriesOffset+=opts.barWidth;if(!(opts.singleSeriesColor)){color=Raphael.leonardo.getPaletteColor();}}}},lineChart:function(domID,series,labels,options){init(domID,getMaxValue(series),labels.length,options||defaultOpts);if(opts.drawBorder){drawBorder(opts.width,opts.height);}
drawXAxis(labels);drawYAxis();for(var item in series){if(series.hasOwnProperty(item)){plot(series[item],function(x,y,value){paper.leonardo.lineTo(x+plotArea.xOffset,y);});}}},sparkline:function(domID,data,options){opts=extend(options,{width:300,height:200,padding:10});plotArea.x=opts.padding;plotArea.width=opts.width-(opts.padding*2);plotArea.height=opts.height;maxValue=Math.max.apply(Math,data);paper=new Raphael(domID,opts.width,opts.height);plot(data,function(x,y,value){if(value===0){y=plotArea.height;}
paper.leonardo.lineTo(x,y);});},oneBar:function(domID,percentageAsDecimal,options){Raphael.leonardo.getPaletteColor({reset:true});opts=extend(options,{width:300,height:200,fillColor:Raphael.leonardo.getPaletteColor()});var paddedHeight=opts.height-2,barWidth=percentageAsDecimal*opts.width,rectAttr={fill:opts.fillColor},text=(percentageAsDecimal*100).toFixed(0)+"%",textAttr={'text-anchor':'start'};paper=new Raphael(domID,opts.width,opts.height);paper.rect(0,1,barWidth,paddedHeight).attr(rectAttr);if(opts.showPercentage){paper.text(2,paddedHeight/2,text).attr(textAttr);}}};}());
...
Raphael.leonardo = {
     ...
     * gives the ability to set hue, saturation and brightness for the initial
     * color instead of just the brightness.
     *
     * If hsb is provided, the "color cycle" will be reset to the color
     * specified and go on from there. This is the equivalent of calling
     * Raphael.getColor.reset() in the standard Raphael library.
     *
     * Arguments:
     * hsb:
     *      An object containing key-value pairs for hue, saturation, and value
     *      (i.e. { h: 0, s: 1, b: 1}). If hsb is null, then the next color in
     *      the cycle is returned.
     */
    getColor: function (hsb) {
        var start = this.getColor.start = hsb ? hsb : this.getColor.start,
            rgb = Raphael.hsb2rgb(start.h, start.s, start.b);

        // TODO: Try to apply some color theory when choosing the next color
        // (i.e. next complimentary color, triad, etc.
        start.h += .075;
        if (start.h > 1) {
            start.h = 0;
            start.s -= .2;
...
Leonardo = (function () {
    ...
    return {
        ...
        columnChart: function (domID, series, labels, options) {
            ...
            color = Raphael.leonardo.getColor(opts.initialColor);
            for (item in series) {
                if (series.hasOwnProperty(item)) {
                    plot(series[item], function (x, y, value) {
                        ...
                        paper.rect(x, y, opts.barWidth, adjustedHeight - y)
                    ...
                    if (!(opts.singleSeriesColor)) {
                        color = Raphael.leonardo.getColor();
                    }
                }
            }
        },
        ...
        oneBar: function (domID, percentageAsDecimal, options) {
            ...
            var paddedHeight = opts.height - 2,
                barWidth     = percentageAsDecimal * opts.width,
                rectAttr     = {fill: Raphael.leonardo.getColor(opts.initialColor)},

...
Raphael.leonardo = {
     ...
     *
     *      reset: true | false => Resets the color cycle index to 0 if true.
     */
    getPaletteColor: function (opts) {
        ...
        if ((!(this.getPaletteColor.index)) || opts.reset) {
            this.getPaletteColor.index = 0;
        }

        var index = this.getPaletteColor.index,
            length = this.getPaletteColor.palette.length,
            color = this.getPaletteColor.palette[index];

        this.getPaletteColor.index = (index === length - 1) ? 0 : index + 1;
...
Leonardo = (function () {
    ...
    return {
        ...
        columnChart: function (domID, series, labels, options) {
            ...
            color = Raphael.leonardo.getPaletteColor({reset: true});
            for (item in series) {
                if (series.hasOwnProperty(item)) {
        ...
        oneBar: function (domID, percentageAsDecimal, options) {
            ...
            var paddedHeight = opts.height - 2,
                ...
                rectAttr     = {fill: Raphael.leonardo.getPaletteColor({reset: true })},
